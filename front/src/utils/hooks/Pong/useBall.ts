...
function useBall(inner: {x: number, y: number}, data: { width: number, height: number, radius: number}, doubleWall: { height: number, bot: number, left: number, right: number}, paddleLeft: {x: number, y: number}, paddleRight: {x: number, y: number}, secondPaddleLeftX: number, secondPaddleRightX: number)
{
		...
		const interval = setInterval(() =>
			{
				...
				else
				{
					...
					setBall(newBall);
					if (dataGame.isDouble && ((newBall.y >= (paddleY.left - data.radius) && newBall.y <= paddleHeight.left && ((newBall.x >= secondPaddleX.left && newBall.x <= secondPaddleMiddleX.left) || (newBall.x <= secondPaddleSideX.left && newBall.x >= secondPaddleMiddleX.left)))
					|| (newBall.x >= secondPaddleX.right && newBall.x <= secondPaddleMiddleX.right && newBall.y >= (paddleY.right - data.radius) && newBall.y <= paddleHeight.right)
					|| (newBall.x <= secondPaddleSideX.right && newBall.x >= secondPaddleMiddleX.right && newBall.y >= (paddleY.right - data.radius) && newBall.y <= paddleHeight.right)))
					{
