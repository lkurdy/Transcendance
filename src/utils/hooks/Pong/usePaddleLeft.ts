...
function usePaddleLeft(inner: {x: number, y: number}, isSecondPaddles: boolean)
{
	const initialPaddleLeft = localStorage.getItem('paddleLeft');
	const [paddleLeft, setPaddleLeftState] = useState<{x: number, y: number}>(initialPaddleLeft ? JSON.parse(initialPaddleLeft) : {x: (isSecondPaddles ? inner.x * 0.557 : inner.y * 0.10), y: inner.y * 0.273});
	const innerBot = useMemo(() => (inner.y * 0.573), [inner.y]);
	const keysPressed = useRef<{ [key: string]: boolean }>({});

	const setPaddleLeft = (value: {x: number, y: number}) => {
		localStorage.setItem('paddleLeft', JSON.stringify(value));
		setPaddleLeftState(value);
	};
